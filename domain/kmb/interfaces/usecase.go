...
type Usecase interface {
	...
	VehicleCheck(manufactureYear string, tenor int, configValue response.DupcheckConfig) (data response.UsecaseApi, err error)
	...
	Scorepro(ctx context.Context, req request.Metrics, pefindoScore, customerSegment string, spDupcheck response.SpDupcheckMap, accessToken string) (responseScs response.IntegratorScorePro, data response.ScorePro, err error)
...
type MultiUsecase interface {
	...
	Ekyc(ctx context.Context, req request.Metrics, cbFound bool, accessToken string) (data response.Ekyc, trxDetail []entity.TrxDetail, err error)
}
