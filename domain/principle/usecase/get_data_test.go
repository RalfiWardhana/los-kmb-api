...
func TestGetDataPrinciple(t *testing.T) {
	...
	sampleTrxKPMInvalid := entity.TrxKPM{
		IDNumber:                "3506124382000087",
		LegalName:               "Customer Wilen Testt",
		MobilePhone:             "085113218192",
		Email:                   "test.email6@gmail.com",
	...
	testcases := []struct {
		...
		},
		{
			...
			expectedResult: map[string]interface{}{
				...
				"referral_code":              sampleTrxKPM.ReferralCode.String,
			},
		},
		{
			name: "success get readjust data - not valid referral code",
			request: request.PrincipleGetData{
				ProspectID: "SAL-123",
				Context:    "Readjust",
			},
			resGetTrxKPM:       sampleTrxKPMInvalid,
			resMDMGetAssetCode: 200,
			resMDMGetAssetBody: `{
				"code": 200,
				"message": "Success",
